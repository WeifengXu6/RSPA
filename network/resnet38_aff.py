...
class Net(network.resnet38d.Net):
    ...
    def forward(self, x, to_dense=False):
        ...
        ff = torch.index_select(x.to(device), dim=2, index=ind_from.cuda(non_blocking=True).to(device))
        ft = torch.index_select(x.to(device), dim=2, index=ind_to.cuda(device =device,non_blocking=True).to(device))
